...
spendable balance is *derived*, never stored, so it cannot conflict on merge. It
is not a plain `lifetime − Σ spends`: lifetime is recomputed from her records and
can fall (a book reopened, a reading day taken back), and two devices that have
not seen each other's purchases each say yes while the merge keeps both — so the
subtraction can go negative. `rewards.js` therefore **replays** the ledger in
order and lets each purchase take only what was there at the time; the shortfall
is written off, never charged to work she does afterwards. Which rooms exist is derived from the ledger. Each room's
