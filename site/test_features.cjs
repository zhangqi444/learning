...
(async () => {
  ...
  check('a finished mock leaves follow-ups on the current week', /Mock follow-up/.test(ckNow), ckNow.match(/Mock follow-up/) ? 'follow-ups present' : 'no Mock follow-up rows');
  ...
  check('and stays on it when the mock was finished late in the evening',
    /Mock follow-up/.test(await body(pg)), 'finish stamped 23:30 local');
